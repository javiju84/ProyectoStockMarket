...
db.once('open',function(callback){
	...
	var REE_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_REE = mongoose.model('IBEX35_REE', REE_schema);

	var urlRee = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_REE.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlRee,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_REE.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_REE(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_REE(aDocs[n=0]);
...
module.exports.IBEX35_REE = IBEX35_REE;