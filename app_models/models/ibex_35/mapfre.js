...
var IBEX35_MAPFRE = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var MAP_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_MAPFRE = mongoose.model('IBEX35_MAPFRE', MAP_schema);

	var urlmapfre = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_MAP.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlmapfre,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_MAPFRE.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_MAPFRE(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_MAPFRE(aDocs[n=0]);
...
module.exports.IBEX35_MAPFRE = IBEX35_MAPFRE;