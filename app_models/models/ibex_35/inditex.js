...
var IBEX35_INDITEX = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var ITX_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_INDITEX = mongoose.model('IBEX35_INDITEX', ITX_schema);

	var urlinditex = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_ITX.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlinditex,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_INDITEX.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_INDITEX(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_INDITEX(aDocs[n=0]);
...
module.exports.IBEX35_INDITEX = IBEX35_INDITEX;