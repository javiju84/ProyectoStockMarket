...
var IBEX35_IBERDROLA = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var IBE_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_IBERDROLA = mongoose.model('IBEX35_IBERDROLA', IBE_schema);

	var urlIBE = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_IBE.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlIBE,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_IBERDROLA.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_IBERDROLA(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_IBERDROLA(aDocs[n=0]);
...
module.exports.IBEX35_IBERDROLA = IBEX35_IBERDROLA;