...
var IBEX35_BBVA = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var BBVA_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_BBVA = mongoose.model('IBEX35_BBVA', BBVA_schema);

	var urlbbva = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_BBVA.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlbbva,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_BBVA.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_BBVA(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_BBVA(aDocs[n=0]);
...
module.exports.IBEX35_BBVA = IBEX35_BBVA;