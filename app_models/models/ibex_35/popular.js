...
var IBEX35_POPULAR = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var POP_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_POPULAR = mongoose.model('IBEX35_POPULAR', POP_schema);

	var urlPopular = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_POP.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlPopular,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_POPULAR.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_POPULAR(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_POPULAR(aDocs[n=0]);
...
module.exports.IBEX35_POPULAR = IBEX35_POPULAR;