...
db.once('open',function(callback){
	...
	var IAG_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_IAG = mongoose.model('IBEX35_IAG', IAG_schema);

	var urlIAG = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_IAG.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlIAG,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_IAG.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_IAG(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_IAG(aDocs[n=0]);
...
module.exports.IBEX35_IAG = IBEX35_IAG;