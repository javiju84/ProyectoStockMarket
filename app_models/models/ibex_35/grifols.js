...
var IBEX35_GRIFOLS = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var GRF_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_GRIFOLS = mongoose.model('IBEX35_GRIFOLS', GRF_schema);

	var urlGrifols = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_GRF.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlGrifols,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_GRIFOLS.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_GRIFOLS(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_GRIFOLS(aDocs[n=0]);
...
module.exports.IBEX35_GRIFOLS = IBEX35_GRIFOLS;