...
var IBEX35_SABADELL = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var SAB_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_SABADELL = mongoose.model('IBEX35_SABADELL', SAB_schema);

	var urlSabadell = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_SAB.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlSabadell,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_SABADELL.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_SABADELL(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_SABADELL(aDocs[n=0]);
...
module.exports.IBEX35_SABADELL = IBEX35_SABADELL;