...
db.once('open',function(callback){
	...
	var FCC_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_FCC = mongoose.model('IBEX35_FCC', FCC_schema);

	var urlFCC = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_FCC.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlFCC,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_FCC.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_FCC(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_FCC(aDocs[n=0]);
...
module.exports.IBEX35_FCC = IBEX35_FCC;