...
var IBEX35_CAIXABANK = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var CABK_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_CAIXABANK = mongoose.model('IBEX35_CAIXABANK', CABK_schema);

	var urlCaixabank = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_CABK.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlCaixabank,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_CAIXABANK.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_CAIXABANK(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_CAIXABANK(aDocs[n=0]);
...
module.exports.IBEX35_CAIXABANK = IBEX35_CAIXABANK;