...
var IBEX35_INDRA = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var IDR_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_INDRA = mongoose.model('IBEX35_INDRA', IDR_schema);
	...
	request({
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_INDRA.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_INDRA(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_INDRA(aDocs[n=0]);
...
module.exports.IBEX35_INDRA = IBEX35_INDRA;