...
var IBEX35_ACCIONA = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var ANA_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_ACCIONA = mongoose.model('IBEX35_ACCIONA', ANA_schema);
	...
	request({
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_ACCIONA.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_ACCIONA(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_ACCIONA(aDocs[n=0]);
...
module.exports.IBEX35_ACCIONA = IBEX35_ACCIONA;