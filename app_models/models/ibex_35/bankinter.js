...
db.once('open',function(callback){
	...
	var BKT_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_BANKINTER = mongoose.model('IBEX35_BANKINTER', BKT_schema);

	var urlBankinter = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_BKT.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlBankinter,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_BANKINTER.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_BANKINTER(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_BANKINTER(aDocs[n=0]);
...
module.exports.IBEX35_BANKINTER = IBEX35_BANKINTER;