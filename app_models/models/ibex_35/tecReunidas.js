...
var IBEX35_TECREUNIDAS = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var TRE_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_TECREUNIDAS = mongoose.model('IBEX35_TECREUNIDAS', TRE_schema);

	var urlReunidas = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_TRE.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlReunidas,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_TECREUNIDAS.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_TECREUNIDAS(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_TECREUNIDAS(aDocs[n=0]);
...
module.exports.IBEX35_TECREUNIDAS = IBEX35_TECREUNIDAS;