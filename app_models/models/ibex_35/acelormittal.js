...
var IBEX35_ARCELORMITTAL = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var MTS_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_ARCELORMITTAL = mongoose.model('IBEX35_ARCELORMITTAL', MTS_schema);

	var urlacelormittal = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_MTS.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlacelormittal,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_ARCELORMITTAL.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_ARCELORMITTAL(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_ARCELORMITTAL(aDocs[n=0]);
...
module.exports.IBEX35_ARCELORMITTAL = IBEX35_ARCELORMITTAL;