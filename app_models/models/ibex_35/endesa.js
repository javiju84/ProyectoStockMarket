...
db.once('open',function(callback){
	...
	IBEX35_ENDESA = mongoose.model('IBEX35_ENDESA', ELE_schema);

	var urlEndesa = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_ELE.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlEndesa,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_ENDESA.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_ENDESA(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_ENDESA(aDocs[n=0]);
...
module.exports.IBEX35_ENDESA = IBEX35_ENDESA;