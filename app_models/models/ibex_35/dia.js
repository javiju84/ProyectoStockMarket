...
db.once('open',function(callback){
	...
	var DIA_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_DIA = mongoose.model('IBEX35_DIA', DIA_schema);

	var urlDia = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_DIA.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlDia,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_DIA.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_DIA(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_DIA(aDocs[n=0]);
...
module.exports.IBEX35_DIA = IBEX35_DIA;