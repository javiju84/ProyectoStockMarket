...
var IBEX35_ENAGAS = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var ENG_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_ENAGAS = mongoose.model('IBEX35_ENAGAS', ENG_schema);

	var urlEnegas = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_ENG.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlEnegas,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_ENAGAS.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_ENAGAS(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_ENAGAS(aDocs[n=0]);
...
module.exports.IBEX35_ENAGAS = IBEX35_ENAGAS;