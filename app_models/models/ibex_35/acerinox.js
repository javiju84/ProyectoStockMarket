...
var IBEX35_ACERINOX = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var ACX_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_ACERINOX = mongoose.model('IBEX35_ACERINOX', ACX_schema);

	var urlAcx = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_ACX.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlAcx,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_ACERINOX.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_ACERINOX(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_ACERINOX(aDocs[n=0]);
...
module.exports.IBEX35_ACERINOX = IBEX35_ACERINOX;