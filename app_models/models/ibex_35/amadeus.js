...
var IBEX35_AMADEUS = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var AMS_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_AMADEUS = mongoose.model('IBEX35_AMADEUS', AMS_schema);

	var urlAmadeus = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_AMS.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlAmadeus,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_AMADEUS.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_AMADEUS(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_AMADEUS(aDocs[n=0]);
...
module.exports.IBEX35_AMADEUS = IBEX35_AMADEUS;