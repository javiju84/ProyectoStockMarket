...
var IBEX35_REPSOL = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var REP_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_REPSOL = mongoose.model('IBEX35_REPSOL', REP_schema);

	var urlRepsol = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_REP.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlRepsol,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_REPSOL.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_REPSOL(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_REPSOL(aDocs[n=0]);
...
module.exports.IBEX35_REPSOL = IBEX35_REPSOL;