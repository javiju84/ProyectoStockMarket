...
db.once('open',function(callback){
	...
	var OHL_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_OHL = mongoose.model('IBEX35_OHL', OHL_schema);

	var urlOhl = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_OHL.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlOhl,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_OHL.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_OHL(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_OHL(aDocs[n=0]);
...
module.exports.IBEX35_OHL = IBEX35_OHL;