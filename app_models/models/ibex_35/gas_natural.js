...
var IBEX35_GASNATURAL = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var GAS_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_GASNATURAL = mongoose.model('IBEX35_GASNATURAL', GAS_schema);

	var urlGasNatural = "https://www.quandl.com/api/v3/datasets/YAHOO/MC_GAS.json?api_key=HGoTu3E3A_Lsv6biw1kc";
	request({
		url: urlGasNatural,
		json: true
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_GASNATURAL.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_GASNATURAL(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_GASNATURAL(aDocs[n=0]);
...
module.exports.IBEX35_GASNATURAL = IBEX35_GASNATURAL;