...
var IBEX35_ABERTIS = null;
db.on("error", console.error.bind(console,"Connection error:"));
db.once('open',function(callback){

	var ABE_schema = mongoose.Schema({
	fecha: String,
	abierto: Number,
	alto: Number,
	bajo: Number,
	...
	IBEX35_ABERTIS = mongoose.model('IBEX35_ABERTIS', ABE_schema);
	...
	request({
		...
		for(var i = 0 ; i < parseo.length; i++){
			...
			//console.log(aDocs)
			IBEX35_ABERTIS.count({},function(error,count){

			if (count === 0){
				for (var n = aDocs.length-1; n >=0 ; n--){
				var docToAdd = new IBEX35_ABERTIS(aDocs[n]);
				...
				for (var n = aDocs.length-1; n >= 0; n--){
				var docToAdd = new IBEX35_ABERTIS(aDocs[n=0]);
...
module.exports.IBEX35_ABERTIS = IBEX35_ABERTIS;